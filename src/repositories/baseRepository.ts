...
export class BaseRepository<T extends Document> implements IBaseRepository<T> {
    ...
    async find(filter: FilterQuery<T> = {}, projection?: Record<string, number>, options?: QueryOptions): Promise<T[]> {
        try {
    ...
    async count(filter: FilterQuery<T>): Promise<number> {
        try {
            ...
            throw new Error(`Count failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
